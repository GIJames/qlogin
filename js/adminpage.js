...
function finishRefresh(){
	...
	for(user in users){
		if(!filtered(users[user])){
			contentsString = contentsString + "<tr><td>" + users[user].name + "</td><td>" + users[user].email + "</td><td>" + users[user].status + "</td><td>" + "<select onchange=\"takeAction(this.value, &quot;" + users[user].name + "&quot;)\" type=\"text\"><option value=\"none\">none</option><option value=\"ban\">ban</option><option value=\"admin\">admin</option><option value=\"delete\">delete</option><option value=\"reset\">reset</option></select>" + "</td></tr>";
		}
	}
